...
int firstMissingNum3(vector<int> &A)
{
    int N = A.size();
    int i =0;
    while(i<N)
    {
        //check if elements are in range 1-N
        ...
        {   
            ...
            if(A[i] != A[swapIdx])
            {
                //put element at its correct position
                swap(A[i], A[swapIdx]);
                i--;
            }
        }
...
int main(void)
{
    ...
    cout<<"is 1 present: "<<count(A.begin(), A.end(), 1)<<endl;
    cout<<firstMissingNum3(A);
